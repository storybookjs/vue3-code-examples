    ...
    (function() {
      ...
      var runInContext = function runInContext2(context) {
        ...
        function baseMerge(object, source2, srcIndex, customizer, stack) {
          ...
          baseFor(source2, function(srcValue, key) {
            ...
            if (isObject2(srcValue)) {
              ...
              var newValue = customizer ? customizer(safeGet(object, key), srcValue, key + "", object, source2, stack) : undefined2;
        ...
        function baseMergeDeep(object, source2, key, srcIndex, mergeFunc, customizer, stack) {
          ...
          var newValue = customizer ? customizer(objValue, srcValue, key + "", object, source2, stack) : undefined2;
...
var extractArgTypes = (component) => {
  ...
  SECTIONS.forEach((section) => {
    ...
    props.forEach(({ propDef, docgenInfo, jsDocTags }) => {
      ...
      const sbType = section === "props" ? (0, import_docs_tools.convert)(docgenInfo) : { name: "void" };
      results[name] = {
        name,
        description,
...
function advancePositionWithClone(pos, source2, numberOfCharacters = source2.length) {
  return advancePositionWithMutation(extend({}, pos), source2, numberOfCharacters);
}
...
function checkCompatEnabled(key, context, loc, ...args2) {
  ...
  if (process.env.NODE_ENV !== "production" && enabled) {
    warnDeprecation(key, context, loc, ...args2);
  }
  return enabled;
}
function warnDeprecation(key, context, loc, ...args2) {
  const val = getCompatValue(key, context);
  if (val === "suppress-warning") {
    return;
  }
  const { message, link } = deprecationData[key];
  const msg = `(deprecation ${key}) ${typeof message === "function" ? message(...args2) : message}${link ? `
  Details: ${link}` : ``}`;
...
function baseParse(content2, options = {}) {
  const context = createParserContext(content2, options);
...
function parseComment(context) {
  ...
  if (!match) {
    content2 = context.source.slice(4);
    ...
    content2 = context.source.slice(4, match.index);
...
function parseBogusComment(context) {
  ...
  if (closeIndex === -1) {
    content2 = context.source.slice(contentStart);
    advanceBy(context, context.source.length);
  } else {
    content2 = context.source.slice(contentStart, closeIndex);
...
function parseAttributeValue(context) {
  ...
  return { content: content2, isQuoted, loc: getSelection(context, start) };
}
...
function parseText(context, mode) {
  ...
  const start = getCursor(context);
  const content2 = parseTextData(context, endIndex, mode);
  return {
    type: 2,
    content: content2,
    loc: getSelection(context, start)
...
function processFor(node, dir, context, processCodegen) {
  ...
  const { source: source2, value, key, index } = parseResult;
  const forNode = {
    type: 11,
    loc: dir.loc,
    source: source2,
...
function createAliasExpression(range, content2, offset) {
  return createSimpleExpression(content2, false, getInnerRange(range, offset, content2.length));
}
...
function createParamsList(args2) {
  ...
  return args2.slice(0, i + 1).map((arg, i2) => arg || createSimpleExpression(`_`.repeat(i2 + 1), false));
}
...
function mapSlots(slotsArgs, generateComponentSource2, slots) {
  return Object.keys(slotsArgs).map((key) => {
    ...
    if ((0, import_vue.isVNode)(slot)) {
      slotContent = generateComponentSource2(slot);
    }
    if (typeof slot === "object" && !(0, import_vue.isVNode)(slot)) {
...
function getTemplateComponents(renderFn, context) {
  try {
    ...
    const story = originalStoryFn ? originalStoryFn(context == null ? void 0 : context.args, context) : context == null ? void 0 : context.component;
...
function generateSlots(context) {
  ...
  const slots = Object.entries(argTypes2).filter(([key, value]) => {
    var _a, _b;
